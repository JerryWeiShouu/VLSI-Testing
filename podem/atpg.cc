...
//Assignment 4
void CIRCUIT::CheckPoint()
{
    int num_checkpoint = No_PI();
    for(unsigned i=0; i<Netlist.size(); i++)
        if(Gate(i)->No_Fanout() > 1)
            num_checkpoint+=Gate(i)->No_Fanout();
    GenerateAllFaultList();

    cout << "Number of faults before collapsing: " << Flist.size() << endl;
    cout << "Number of faults with checkpoint theorm: " << num_checkpoint * 2 << endl;
    cout << "Percentage of collapsed faults: " << (1 - float(num_checkpoint*2)/float(Flist.size())) * 100 << "%" << endl;
}
...
void CIRCUIT::GenerateBridgingFaultList()
{
    ...
    for(unsigned i = 0;i <= MaxLevel;i++) {
        ...
        while (!Queue[i].empty()){
            gptr1 = Queue[i].front();
            Queue[i].pop_front();
            if(gptr1->GetFunction()!=G_PO){
                Q_wopo.push_back(gptr1);
            }
            gptr1->ResetFlag(SCHEDULED);
        ...
        while (!Q_wopo.empty()){
            ...
            gptr2 = Q_wopo.front();
            if(!Q_wopo.empty()){
            fptr = new BFAULT(gptr1,gptr2,false);//and bridging
            BFlist.push_back(fptr);
            fptr = new BFAULT(gptr1,gptr2,true);//or bridging
            BFlist.push_back(fptr);
            }
        }
    }
...
void CIRCUIT::CreateBridgingFaultListOutputFile(const char *outputfile)
{
    cout << "Create the bridging fault list output file" << endl;
    GenerateBridgingFaultList();

    ofstream ofile(outputfile, ios::out);

    list<BFAULT*>::iterator it_bfault = BFlist.begin();
    for(; it_bfault != BFlist.end(); it_bfault++)
    {
        ofile << "(" << (*it_bfault)->GetGate1()->GetName() << ", " << (*it_bfault)->GetGate2()->GetName();
        if((*it_bfault)->BridgingType())
            ofile << ", OR)" << endl;
        else
            ofile << ", AND)" << endl;
    }
...
//Assignment 6
void CIRCUIT::GenerateNetFaultList(string net)
{
    register unsigned i;
    //GATEFUNC fun;
    FAULT *fptr;
...
void CIRCUIT::RandomPatternPODEM()
{
    cout << "Run the PODEM with random pattern" << endl;
    double rand_max = 2.0;
    unsigned rand_pattern_num(0);
    unsigned i;
    ...
    while(rand_pattern_num < 1000 && UFlist.size() > 0.1*Flist.size())
    {
        for (i=0; i<PIlist.size(); i++)
        {
            int rand_value = (int)(((double)rand()/(double)(RAND_MAX + 1.0)));
            if(rand_value == 0)
    ...
    cout << "random pattern num = " << rand_pattern_num << endl;
    cout << "current fault coverage [1 - (Length of UFList)/(Length of FList) ] = " << 1.00- double(UFlist.size())/double(Flist.size()) << endl;
    cout << "Run stuck-at fault ATPG" << endl;
    unsigned  total_backtrack_num(0), pattern_num(0);
    ATPG_STATUS status;
    FAULT* fptr;
    list<FAULT*>::iterator fite;
    ...
    cout.precision(2);
    cout << "---------------------------------------" << endl;
    cout << "Toal test pattern number = " << pattern_num+rand_pattern_num << endl;
    cout << "Total backtrack number = " << total_backtrack_num << endl;
    cout << "---------------------------------------" << endl;
    cout << "Total fault number = " << total_num << endl;
    cout << "Detected fault number = " << detected_num << endl;
